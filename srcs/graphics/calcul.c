...
void	calculate_line_height_and_draw_start_end(t_data *data)
{
	...
	data->ray.draw_start = -data->ray.line_height / 2 + screenHeight / 2 + data->ray.pitch;
	if (data->ray.draw_start < 0)
		data->ray.draw_start = 0;
	data->ray.draw_end = data->ray.line_height / 2 + screenHeight / 2 + data->ray.pitch;
...
{
	if (side == 0)
		data->ray.wallX = data->player.pos_x + data->ray.pw_dist * data->ray.rayDirY;
	else
		data->ray.wallX = data->player.pos_y + data->ray.pw_dist * data->ray.rayDirX;
...
void	calculate_tex_x(t_data *data, int side)
{
	...
	if ((side == 0 && data->ray.rayDirX > 0) || (side == 1 && data->ray.rayDirY < 0))
